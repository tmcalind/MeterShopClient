...
import { 
    createMap, getMetersFeatureLayer, getGraphicsLayer, 
    tasksSymbol, lettersSymbol, callsSymbol, scheduledSymbol, shutoffsSymbol,
    hydroSymbol, crmSymbol 
} from "./MapTools";
...
const MeterMap = ({
        ...
        const hydroServiceOrderObjectIds = useSelector(state => state.lists.hydroServiceOrderObjectIds)
        const hydroServiceOrderObjectIdsVisible = useSelector(state => state.lists.hydroServiceOrderObjectIdsVisible)

        const hydroNotificationObjectIds = useSelector(state => state.lists.hydroNotificationObjectIds)
        const hydroNotificationObjectIdsVisible = useSelector(state => state.lists.hydroNotificationObjectIdsVisible)
            ...
            if (mapDiv.current) {
                ...
                if (cityworksTasksVisible) {
                    if (cityworksTasks && cityworksTasks.length > 0) {
                        const cityworksTasksObjectIds = cityworksTasks.map((task) => task.ObjectId)
                        console.log('tasks object ids', cityworksTasksObjectIds)
                        map.add(
                            getGraphicsLayer(cityworksTasksObjectIds, meterFeatureLayer, tasksSymbol, 'cityworksTasks')
                        )
                    }
                ...
                if (hydroServiceOrderObjectIdsVisible) {
                    if (hydroServiceOrderObjectIds && hydroServiceOrderObjectIds.length > 0) {
                        map.add(
                            getGraphicsLayer(hydroServiceOrderObjectIds, meterFeatureLayer, hydroSymbol, 'hydroServiceOrders')
                        )
                    }
                }

                if (hydroNotificationObjectIdsVisible) {
                    if (hydroNotificationObjectIds && hydroNotificationObjectIds.length > 0) {
                        map.add(
                            getGraphicsLayer(hydroNotificationObjectIds, meterFeatureLayer, hydroSymbol, 'hydroNotifications')
                        )
                    }
                }
                ...
                if (crmObjectIdsVisible) {
                    if (crmObjectIds && crmObjectIds.length > 0) {
                        map.add(
                            getGraphicsLayer(crmObjectIds, meterFeatureLayer, crmSymbol, 'crm')
                        )
                    }
                }
                ...
                if (lettersObjectIdsVisible) {
                    if (lettersObjectIds && lettersObjectIds.length > 0) {
                        const lettersLayer = getGraphicsLayer(lettersObjectIds, meterFeatureLayer, lettersSymbol, 'letters')
                ...
                if (callsObjectIdsVisible) {
                    if (callsObjectIds && callsObjectIds.length > 0) {
                        const callsLayer = getGraphicsLayer(callsObjectIds, meterFeatureLayer, callsSymbol, 'calls')
                        map.add(callsLayer)
                    }
                }
                ...
                if (scheduledObjectIdsVisible) {
                    if (scheduledObjectIds && scheduledObjectIds.length > 0) {
                        const scheduledLayer = getGraphicsLayer(scheduledObjectIds, meterFeatureLayer, scheduledSymbol, 'scheduled')
                ...
                if (shutoffsObjectIdsVisible) {
                    if (shutoffsObjectIds && shutoffsObjectIds.length > 0) {
                        const shutoffsLayer = getGraphicsLayer(shutoffsObjectIds, meterFeatureLayer, shutoffsSymbol, 'shutoffs')
                            ...
                            if (event.results) {
                                event.results.forEach((result) => {
                                    if (result.graphic.layer.name) {
                                        console.log(result.graphic.layer.name, result.graphic.attributes)
                                        dispatch(
                                            setWorkOrder(
                                                { 
